...
static void assemble(const string& pathIn, const string& pathOut)
{
	...
	RTimer *rtimer = new RTimer("SAS_LOADING");
	if (!pathIn.empty())
...
erode:
	...
	rtimer = new RTimer("SAS_POP_BUBBLES");
	if (opt::bubbleLen > 0)
		popBubbles(g);
	...
	rtimer = new RTimer("SAS_ASSEMBLE");
